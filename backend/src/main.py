...
class SimEngine:
    ...
    def _fsm_tick(self):
        ...
        near_obs = any(
            math.hypot(d["x"] - (k % COLS) * CELL - CELL / 2,
                       d["y"] - (k // COLS) * CELL - CELL / 2) < CELL * 1.5
    ...
    def tick(self):
        ...
        if active and self.goal:
            ...
            dx, dy = self.rl.correct(self.drone, target, self.global_path, CELL, self.rl_enabled, self.speed)
            ...
            if 0 <= nr < ROWS and 0 <= nc < COLS and self.world.grid[nr][nc] == 1:
                ...
                self.drone["vx"] *= -0.6
                self.drone["vy"] *= -0.6
                nx = self.drone["x"] + self.drone["vx"]
                ny = self.drone["y"] + self.drone["vy"]
                if self.rl_enabled:
                    self.rl.reward -= 1
