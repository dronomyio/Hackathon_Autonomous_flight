...
export default function Sidebar({ simState }) {
  const logRef = useRef(null);
  const logEntriesRef = useRef([]);

  useEffect(() => {
    if (!simState?.events?.length) return;
    simState.events.forEach(ev => {
      logEntriesRef.current.push(ev);
    });
    ...
    if (logRef.current) {
      ...
      box.innerHTML = logEntriesRef.current.map(e =>
        `<div class="log-entry log-${e.kind}">[${e.t}] ${e.msg}</div>`
      ).join('');
  ...
  if (!simState) {
    return (
      <div className={styles.sidebar}>
        <div className={styles.connecting}>Connecting to simulation...</div>
      </div>
    );
  }
    ...
    <div className={styles.sidebar}>
      ...
      <Section title="EKF estimator">
        <MetricRow
          ...
          value={`${Math.hypot(drone.x - ekf.x, drone.y - ekf.y).toFixed(1)} m`}
          color="#185FA5"
        />
        <Bar value={Math.hypot(drone.x - ekf.x, drone.y - ekf.y)} color="#185FA5" max={30} />
        <MetricRow
          label="Velocity"
          value={`(${drone.vx.toFixed(1)}, ${drone.vy.toFixed(1)})`}
        />
        <MetricRow
          label="Innovation"
          value={ekf.innovation.toFixed(2)}
