...
export default function WorldCanvas({ simState, onCanvasClick }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Grid lines
      ctx.strokeStyle = 'rgba(80,110,150,0.07)'; ctx.lineWidth = 0.5;
      ...
      // START label
      ctx.fillStyle = 'rgba(80,200,150,0.6)'; ctx.font = '10px monospace';
      ctx.fillText('START', 14, 13);

      // HUD strip
      ctx.fillStyle = 'rgba(13,17,23,0.55)'; ctx.fillRect(3, 3, 210, 17);
      ctx.fillStyle = 'rgba(80,200,150,0.7)'; ctx.font = '10px monospace';
  ...
  return (
    <canvas
      ...
      height={520}
      onClick={handleClick}
      style={{ display: 'block', cursor: 'crosshair', background: '#0d1117' }}
    />
  );
}
